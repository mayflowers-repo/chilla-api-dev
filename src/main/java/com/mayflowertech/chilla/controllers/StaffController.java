...
public class StaffController {
    ...
    public ApiResult<List<Student>> getStudents(Model model) {
	  ...
	  @ApiOperation(value = "Add or edit services for a student")
	  @ApiResponses(value = {
	      @ApiResponse(code = 200, message = "Successfully updated student services"),
	      @ApiResponse(code = 404, message = "Student or service not found")
	  })
	  @RequestMapping(value = "/updatestudentservices", method = RequestMethod.PUT, consumes = MediaType.APPLICATION_JSON_VALUE)
	  public ApiResult<StudentPojo> updateStudentServices(
		  ...
		  try {
		        ...
		        Student updatedStudent = studentService.updateStudentServices(studentPojo.getStudentId(), studentPojo.getServices());
		    	...
		    	e.printStackTrace();
		        return new ApiResult<>(HttpStatus.INTERNAL_SERVER_ERROR.value(), "An unexpected error occurred", studentPojo);
		    }
	  }
	  
	  @ApiOperation(value = "Get details of a student by ID")
	    @ApiResponses(value = {
	  ...
	  public ApiResult<StudentPojo> updateStudentById(@PathVariable Long studentId, @RequestBody StudentPojo studentPojo) {
	  ...
	  public ApiResult<ManagerPojo> updateManagerById(@PathVariable Long managerId, @RequestBody ManagerPojo managerPojo) {
	  ...
	  @Secured({"ROLE_MANAGER", "ROLE_ADMIN", "ROLE_SYSTEMADMIN"})
	    ...
	    @ApiResponses(value = {
	        ...
	        @ApiResponse(code = 404, message = "Manager, booking request, or student not found")
	    })
	    ...
	    public ApiResult<BookingRequestAssignPojo> assignStudentsToBookingRequest(
	        ...
	        try {
	            ...
	            BookingRequest updatedBookingRequest = managerService.assignStudentsToBookingRequest(
	                bookingRequestPojo.getManagerId(),
	                bookingRequestPojo.getBookingRequestId(),
	                bookingRequestPojo.getStudentIds()
	            );

	            return new ApiResult<>(HttpStatus.OK.value(), "Students assigned to booking request successfully", bookingRequestPojo);

	        } catch (CustomException e) {
	            return new ApiResult<>(HttpStatus.NOT_FOUND.value(), e.getMessage(), bookingRequestPojo);

	        } catch (Exception e) {
	            return new ApiResult<>(HttpStatus.INTERNAL_SERVER_ERROR.value(), "An unexpected error occurred", bookingRequestPojo);
	        }
	    }
	    
	    
	    @ApiOperation(value = "Get patients assigned to a student")
	    @ApiResponses(value = {
	        @ApiResponse(code = 200, message = "Successfully fetched patients"),
	    ...
	    public ApiResult<List<PatientPojo>> getPatientsAssignedToStudent(
	        ...
	        try {
	            ...
	            List<PatientPojo> patientPojos = patients.stream().map(patient -> new PatientPojo(patient)).collect(Collectors.toList());
