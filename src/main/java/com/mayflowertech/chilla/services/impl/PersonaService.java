	...
	@Override
	  public User signUpUser(UserSignupPojo persona) throws CustomException {
        ...
        if (userService.isExist(user)) {
            throw new CustomException("Username/email already taken: " + user.getEmail());
        }
        ...
        if (SystemRoles.CUSTOMER.getRoleCode().equalsIgnoreCase(persona.getPersona())) {
            ...
            String otp = mailService.generateOtp(persona.getEmail());
            logger.info("sending otp "+otp+" to "+persona.getEmail());
            //mailService.sendVerificationEmail(persona.getEmail(), "User", otp, null);
